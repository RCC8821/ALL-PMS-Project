...
router.get('/check-in-today', async (req, res) => {
  try {
    ...
    // IN entries sheet से पढ़ो
    // valueRenderOption: 'FORMATTED_VALUE' → जैसा sheet में दिख रहा है वैसा आएगा
    const result = await sheets.spreadsheets.values.get({
      spreadsheetId: LABOUR_ID,
      range: 'REACTFORMDATA!A:Q',  // IN sheet का नाम और range तुम्हारे हिसाब से एडजस्ट करो
      valueRenderOption: 'FORMATTED_VALUE',  // ← महत्वपूर्ण बदलाव
      dateTimeRenderOption: 'FORMATTED_STRING',
    });

    const rows = result.data.values || [];
    if (rows.length <= 1) {
      return res.json({ success: true, hasIn: false });
    }

    // Header skip करके search
    const found = rows.slice(1).find(row => {
      let submitDate = row[0]; // A column = Submit Date

      // अगर date string अलग format में आ रही है तो normalize करो
      if (submitDate) {
        // अगर DD/MM/YYYY या DD-MM-YYYY है तो YYYY-MM-DD में बदलो
        if (typeof submitDate === 'string') {
          submitDate = submitDate.trim();

          // DD/MM/YYYY या DD-MM-YYYY → YYYY-MM-DD
          const parts = submitDate.match(/(\d{1,2})[/-](\d{1,2})[/-](\d{4})/);
          if (parts) {
            const [_, dd, mm, yyyy] = parts;
            submitDate = `${yyyy}-${mm.padStart(2, '0')}-${dd.padStart(2, '0')}`;
          }
        }

        // अब comparison
        const nameInSheet = row[4]?.trim().toLowerCase(); // E column = Name (adjust index if needed)
        return submitDate === today && nameInSheet === name.trim().toLowerCase();
      }
      return false;
    });
    ...
    res.json({
      ...
      hasIn: true,
      data: {
        siteName: found[2] || '',    // C = Site Name (adjust index)
        category: found[6] || '',    // G = Category
        inTime:   found[10] || '',   // K = In Time (adjust according to your sheet)
      }
    });

  } catch (err) {
    console.error('Check-in-today error:', err);
    res.status(500).json({ success: false, message: 'Server error while checking IN' });
  }
});
