...
// ==========================================
// ⚡ IMAGE COMPRESSION — send karne se pehle
// Frontend pe hi compress karo — kam data jaayega
// ==========================================
const compressImage = (dataUrl, maxWidth = 320, quality = 0.7) => {
...
const findNearestSite = (lat, lng) => {
  let nearest = null, minDist = Infinity;
  for (const site of SITE_LOCATIONS) {
    const d = calculateDistance(lat, lng, site.lat, site.lng);
    if (d < minDist) { minDist = d; nearest = site; }
  }
  return { site: nearest, distance: minDist };
};

const checkSiteDistance = (lat, lng, siteName) => {
  const selected = SITE_LOCATIONS.find(s => s.name.toLowerCase() === siteName.toLowerCase());
  if (selected) {
    const d = calculateDistance(lat, lng, selected.lat, selected.lng);
    return { distance: d, isWithin: d <= 300 };
  }
  const { site, distance } = findNearestSite(lat, lng);
  return { distance, isWithin: distance <= 300, nearestSite: site?.name };
};
...
const LabourAttendance = () => {
  ...
  const [attendanceData, setAttendanceData] = useState({ site: '', labourType: '', category: '', contractorName: '' });
  const [formData, setFormData] = useState({ name: '', phone: '', labourType: '', category: '', site: '', contractorName: '' });
  const [dropdownData, setDropdownData] = useState({ siteNames: [], laborTypes: [], categories: [], contractorNames: [] });
  const [attendanceResult, setAttendanceResult] = useState(null);

  // Registration angle guidance
  const angleGuides = [
    { text: 'Photo 1: सीधे camera की तरफ देखो', icon: '😐' },
    { text: 'Photo 2: थोड़ा बाईं तरफ मुड़ो', icon: '😶' },
    { text: 'Photo 3: थोड़ा दाईं तरफ मुड़ो', icon: '😶' },
  ];
  ...
  useEffect(() => {
    fetchDropdownData();
    warmupServer(); // ⚡ App open hone par server warm karo
  }, []);
  ...
  useEffect(() => {
    if (message) {
      if (msgTimerRef.current) clearTimeout(msgTimerRef.current);
      msgTimerRef.current = setTimeout(() => { setMessage(''); setMessageType(''); }, 5000);
    }
    return () => clearTimeout(msgTimerRef.current);
  }, [message]);
  ...
  const warmupServer = async () => {
    try {
      const res = await axios.get(`${API_URL}/warmup`, { timeout: 30000 });
      if (res.data.success) {
        setServerReady(true);
        console.log('✅ Server ready, cache:', res.data.cacheSize);
      }
    } catch (error) {
  ...
  const fetchDropdownData = async () => {
    try {
      const response = await axios.get(`${API_URL}/dropdown-data`);
      if (response.data.success) setDropdownData(response.data.data);
    } catch (error) {
  ...
  const getCurrentLocation = () => new Promise((resolve, reject) => {
    if (!navigator.geolocation) { reject(new Error('GPS supported nahi hai')); return; }
    navigator.geolocation.getCurrentPosition(
      (pos) => resolve({ lat: pos.coords.latitude, lng: pos.coords.longitude, accuracy: pos.coords.accuracy }),
      (err) => {
        const msgs = { 1: 'GPS permission denied. Settings mein allow karo.', 2: 'Location unavailable.', 3: 'Location timeout. Dobara try karo.' };
  ...
  const fetchUserLocation = async () => {
    ...
    try {
      ...
      showMessage(
        distance <= 300
          ? `📍 Location OK! ${site?.name} (${Math.round(distance)}m)`
          : `⚠️ ${Math.round(distance)}m door ho site se`,
        distance <= 300 ? 'success' : 'error'
      );
      return location;
    } catch (error) {
  ...
  const openCamera = async (mode = facingMode) => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        // ⚡ Smaller resolution — kam data, faster capture
      ...
      showMessage('Camera permission do. Settings mein jaao.', 'error');
    }
  };
  ...
  // ⚡ Capture + compress — low bandwidth
  const captureFrame = useCallback(async () => {
    ...
    cvs.height = video.videoHeight;
    cvs.getContext('2d').drawImage(video, 0, 0);
    const rawDataUrl = cvs.toDataURL('image/jpeg', 0.9);
    // ⚡ Compress: 320px max, quality 0.7 — ~15-30KB per image
    return await compressImage(rawDataUrl, 320, 0.7);
  }, []);

  // ==========================================
  // HELPERS
  // ==========================================
  const showMessage = (msg, type = '') => { setMessage(msg); setMessageType(type); };
  ...
  const resetAll = () => {
    ...
    setRecognizedLabour(null); setTodayAttendance(null); setAttendanceResult(null);
    setAttendanceStep('capture');
    setUserLocation(null); setNearestSite(null); setDistanceToSite(null); setIsWithinRange(false);
  };
  ...
  // ==========================================
  const handleRecognizeFace = async () => {
    if (!cameraOpen) { showMessage('Pehle camera kholo', 'error'); return; }

    setLoading(true);
    showMessage('Face scan + location ho raha hai...', '');

    // Pehle sab saaf karo — kisi ka bhi data nahi dikhega jab tak match na ho
    setRecognizedLabour(null);
    setTodayAttendance(null);
    setAttendanceData({ site: '', labourType: '', category: '', contractorName: '' });
    setAttendanceResult(null);
    ...
    try {
      ...
      if (!photo) { showMessage('Photo capture fail. Dobara try karo.', 'error'); setLoading(false); return; }
      ...
      if (location.error) {
        showMessage(`Location nahi mila: ${location.error}`, 'error');
        setLoading(false);
        return;
      }
      ...
      // ✅ Sirf tabhi kuch dikhao jab server ne confirm kiya ho
      if (response.data.success && response.data.labour) {
        const labour = response.data.labour;

        // Frontend par bhi confidence check — 65% se kam = reject
        if (!labour.matchConfidence || labour.matchConfidence < 65) {
          showMessage('❌ Yeh chehra register nahi hai. Pehle register karo.', 'error');
          if (navigator.vibrate) navigator.vibrate([100, 50, 100]);
          setLoading(false);
          return;
        }
        ...
        // 65%+ match — sahi banda
        setRecognizedLabour(labour);
        setTodayAttendance(response.data.todayAttendance);
        setAttendanceData({
          site: labour.defaultSite || '',
          labourType: labour.defaultLabourType || '',
          category: labour.defaultCategory || '',
          contractorName: labour.contractorName || '',
        });

        if (response.data.todayAttendance?.checkOutTime) {
          setAttendanceResult({ status: 'completed', ...response.data.todayAttendance });
          setAttendanceStep('done');
          showMessage('Aaj ka attendance pura ho chuka hai', 'success');
        } else {
          setAttendanceStep('select');
          showMessage(
            distance <= 300
              ? `✅ ${labour.name} pehchana gaya! (${Math.round(distance)}m)`
              : `⚠️ ${labour.name} — site se ${Math.round(distance)}m door ho`,
            distance <= 300 ? 'success' : 'error'
          );
        }
        if (navigator.vibrate) navigator.vibrate(200);
      ...
      // Error = match nahi hua — inputs bilkul blank rahenge
      setRecognizedLabour(null);
      setTodayAttendance(null);
      setAttendanceData({ site: '', labourType: '', category: '', contractorName: '' });

      const serverMsg = error.response?.data?.error;
      showMessage(serverMsg || '❌ Yeh chehra register nahi hai. Pehle register karo.', 'error');
      if (navigator.vibrate) navigator.vibrate([100, 50, 100]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmitAttendance = async () => {
    if (!attendanceData.site) { showMessage('Site chuniye', 'error'); return; }
    if (!attendanceData.labourType) { showMessage('Labour type chuniye', 'error'); return; }
    if (!userLocation) { showMessage('Location nahi mila', 'error'); return; }

    const locationCheck = checkSiteDistance(userLocation.lat, userLocation.lng, attendanceData.site);
    if (!locationCheck.isWithin) {
      showMessage(`❌ ${Math.round(locationCheck.distance)}m door ho. 300m ke andar aao.`, 'error');
      return;
    }

    setLoading(true);
    showMessage('Attendance mark ho raha hai...', '');
    ...
    try {
      const response = await axios.post(`${API_URL}/mark-attendance`, {
        ...
        category: attendanceData.category,
        contractorName: attendanceData.contractorName,
        location: { lat: userLocation.lat, lng: userLocation.lng, accuracy: userLocation.accuracy, distance: Math.round(locationCheck.distance) },
      }, { timeout: 15000 });
      ...
      showMessage(error.response?.data?.error || 'Attendance mark nahi hua.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const captureForRegistration = async () => {
    if (!cameraOpen) { showMessage('Pehle camera kholo', 'error'); return; }
    if (capturedPhotos.length >= 3) { showMessage('3 photos ho gayi hain', 'success'); return; }

    const photo = await captureFrame();
    if (!photo) { showMessage('Photo fail. Dobara try karo.', 'error'); return; }

    setCapturedPhotos(prev => {
      const updated = [...prev, photo];
      const guide = angleGuides[prev.length];
      showMessage(guide ? `✅ ${guide.text}` : `Photo ${updated.length} li!`, 'success');
      return updated;
    });
  };
  ...
  const removePhoto = (index) => {
    setCapturedPhotos(prev => {
      const updated = prev.filter((_, i) => i !== index);
      showMessage(`Photo hatayi. ${updated.length} baaki hain.`, 'success');
      return updated;
    });
  };

  const handleRegister = async () => {
    if (capturedPhotos.length < 2) { showMessage('Kam se kam 2 photos lo', 'error'); return; }
    if (!formData.name.trim()) { showMessage('Naam likhiye', 'error'); return; }
    if (formData.phone.length < 10) { showMessage('10 digit ka phone number likhiye', 'error'); return; }
    if (!formData.labourType) { showMessage('Labour type chuniye', 'error'); return; }

    setLoading(true);
    showMessage('Register ho raha hai...', '');
    ...
    try {
      ...
      // ⚡ Photos already compressed hain
      for (let i = 0; i < capturedPhotos.length; i++) {
        const blob = await base64ToBlob(capturedPhotos[i]);
        fd.append('photos', blob, `photo${i}.jpg`);
      }
      ...
      setFormData({ name: '', phone: '', labourType: '', category: '', site: '', contractorName: '' });
      if (navigator.vibrate) navigator.vibrate(200);
      setTimeout(() => { setMode('attendance'); resetAll(); }, 3000);
    } catch (error) {
      showMessage(error.response?.data?.error || 'Register nahi hua.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  // RENDER HELPERS
  // ==========================================
  const renderDropdown = ({ label, name, value, options, onChange, required = false }) => (
    <div>
      <label className="block text-gray-700 font-medium text-sm mb-1">
        {label} {required && <span className="text-red-500">*</span>}
      </label>
      <select
        name={name} value={value} onChange={onChange} disabled={loading}
        className="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:border-gray-500 outline-none transition-all disabled:opacity-50 bg-white"
      >
        <option value="">— {label} chuniye —</option>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-5">
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            <span className={`w-2 h-2 rounded-full ${serverReady ? 'bg-green-500' : 'bg-yellow-400 animate-pulse'}`}></span>
            <span className="text-xs text-gray-500">
              {serverReady ? 'Server ready — fast scan possible' : 'Server warm ho raha hai...'}
            </span>
          </div>
            ...
            {[
              ...
              <button key={key}
                className={`px-5 py-2 rounded-full font-semibold text-sm border transition-all ${
                  mode === key ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-600 border-gray-300'
                }`}
                onClick={() => { setMode(key); resetAll(); }}
              >{label}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <h3 className="text-sm font-semibold text-gray-700">
                  {mode === 'attendance' ? '📸 Face Scan' : '📷 Photos'}
                </h3>
              ...
              {!cameraOpen ? (
                <div
                  ...
                  onClick={() => openCamera()}
                >
                  <div className="w-16 h-16 rounded-full bg-white border border-gray-200 flex items-center justify-center text-3xl mb-3 shadow-sm">📷</div>
                  <p className="text-gray-600 font-semibold">Camera kholne ke liye tap karo</p>
                  <p className="text-gray-400 text-sm mt-1">Front/back dono camera available hain</p>
                </div>
              ) : (
                <div className="relative bg-black" style={{ height: '300px' }}>
                  ...
                  {/* Face guide oval */}
                  <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                    <div className="border-2 border-green-400 rounded-full opacity-60" style={{ width: '140px', height: '180px' }}></div>
                  </div>
                  ...
                  {/* Camera controls */}
                  <button className="absolute top-3 right-3 bg-black bg-opacity-50 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm hover:bg-opacity-80 z-10" onClick={stopCamera}>✕</button>
                  <button className="absolute top-3 left-3 bg-black bg-opacity-50 text-white rounded-full w-8 h-8 flex items-center justify-center text-lg hover:bg-opacity-80 z-10" onClick={toggleCamera} title="Camera badlo">🔄</button>
                </div>
              )}

              {/* Registration angle guide */}
              {mode === 'register' && cameraOpen && capturedPhotos.length < 3 && (
                <div className="px-4 py-2 bg-blue-50 border-t border-blue-100">
                  <p className="text-blue-800 text-xs font-medium text-center">
                    {angleGuides[capturedPhotos.length]?.icon} {angleGuides[capturedPhotos.length]?.text}
                  </p>
                  {/* Progress dots */}
                  <div className="flex justify-center gap-2 mt-1.5">
                    {[0, 1, 2].map(i => (
                      <div key={i} className={`w-2 h-2 rounded-full ${i < capturedPhotos.length ? 'bg-green-500' : i === capturedPhotos.length ? 'bg-blue-400 animate-pulse' : 'bg-gray-300'}`}></div>
                    ))}
                  </div>
                </div>
              )}
                  ...
                  attendanceStep === 'capture' ? (
                    <button
                      className={`w-full py-3 px-4 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                        loading ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-gray-700 active:scale-[0.98]'
                      }`}
                      onClick={handleRecognizeFace}
                      disabled={loading}
                    >
                      {loading ? (
                        <><div className="w-4 h-4 border-2 border-gray-400 border-t-gray-600 rounded-full animate-spin"></div>Scan ho raha hai...</>
                      ) : '📸 Face + Location Scan Karo'}
                    </button>
                  ) : (
                    <button
                      className="w-full py-3 px-4 rounded-xl font-semibold text-sm bg-gray-200 text-gray-600"
                      onClick={() => { resetAll(); setAttendanceStep('capture'); }}
                    >🔄 Naya Scan</button>
                  )
                ) : (
                  <button
                    className={`w-full py-3 px-4 rounded-xl font-bold text-sm transition-all ${
                      loading || capturedPhotos.length >= 3 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-gray-700 active:scale-[0.98]'
                    }`}
                    onClick={captureForRegistration}
                    disabled={loading || capturedPhotos.length >= 3}
                  >
                    📸 Photo Lo ({capturedPhotos.length}/3)
                  </button>
                )}
              </div>
            </div>
            ...
            {/* Location status */}
            {mode === 'attendance' && userLocation && (
              <div className={`mt-3 rounded-xl border p-3 ${isWithinRange ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-center gap-3">
                  <span className="text-xl">{isWithinRange ? '📍' : '⚠️'}</span>
                  <div className="flex-1">
                    <h4 className={`font-semibold text-sm ${isWithinRange ? 'text-green-800' : 'text-red-800'}`}>
                      {isWithinRange ? 'Location Verify Ho Gayi' : 'Site Se Zyada Door Ho'}
                    </h4>
                    <p className={`text-xs ${isWithinRange ? 'text-green-600' : 'text-red-600'}`}>
                      {nearestSite ? `${nearestSite.name} — ${distanceToSite}m` : 'Calculate ho raha hai...'}
                    </p>
                  </div>
                  <span className={`text-lg font-bold ${isWithinRange ? 'text-green-700' : 'text-red-700'}`}>{distanceToSite}m</span>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Right Panel */}
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
              ...
              {mode === 'attendance' ? (
                ...
                <div className="space-y-4">
                  {attendanceStep === 'capture' && (
                    <>
                      <div>
                        <h2 className="text-lg font-bold text-gray-800">Attendance Mark Karo</h2>
                        <p className="text-gray-500 text-xs mt-0.5">Camera kholo → Face scan karo</p>
                      </div>
                      <div className="bg-blue-50 border-l-4 border-blue-400 rounded-r-lg p-3">
                        <p className="text-blue-900 font-semibold text-xs mb-1.5">📋 Jaruri cheezein</p>
                        {['Seedha camera ki taraf dekho', 'GPS on rakho', 'Site se 300m ke andar raho', 'Achhi roshni mein aao'].map(tip => (
                          <div key={tip} className="flex items-center gap-1.5 text-blue-800 text-xs mb-1">
                      ...
                      {/* Sites list */}
                      <div className="bg-gray-50 rounded-xl p-3 border border-gray-200">
                        <p className="text-gray-500 text-xs font-medium mb-2">📍 Sites</p>
                        <div className="max-h-28 overflow-y-auto space-y-1">
                          {SITE_LOCATIONS.map((site, idx) => (
                            <div key={idx} className="text-xs text-gray-500 py-0.5 border-b border-gray-100 last:border-0">{site.name}</div>
                          ))}
                        </div>
                      </div>
                    </>
                    ...
                    <>
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                        <div className="flex items-center gap-3 mb-2">
                          {recognizedLabour.imageUrl && recognizedLabour.imageUrl !== 'N/A' && (
                            <img src={recognizedLabour.imageUrl} alt={recognizedLabour.name} className="w-10 h-10 rounded-full object-cover border-2 border-green-300" />
                          )}
                          <div>
                            <h3 className="font-bold text-green-800">{recognizedLabour.name}</h3>
                            <p className="text-green-600 text-xs">{recognizedLabour.phone}</p>
                          </div>
                          <span className="ml-auto text-green-600 text-xl">✅</span>
                        </div>
                        <p className="text-green-700 text-xs">Match: {recognizedLabour.matchConfidence?.toFixed(1)}%</p>
                      </div>
                      ...
                      {!isWithinRange && (
                        <div className="bg-red-50 border border-red-300 rounded-xl p-3">
                          <div className="flex items-start gap-2 text-red-800">
                            <span>🚫</span>
                            <div>
                              <p className="font-bold text-sm">Attendance Nahi Milega</p>
                              <p className="text-xs">{distanceToSite}m door ho site se. 300m ke andar aao.</p>
                            </div>
                          </div>
                          <button className="mt-2 w-full py-2 text-xs bg-red-100 text-red-700 rounded-lg" onClick={fetchUserLocation} disabled={locationLoading}>
                            {locationLoading ? '⏳ Location mil rahi hai...' : '📍 Location Dobara Check Karo'}
                          </button>
                        </div>
                      )}
                      ...
                      {todayAttendance?.isCheckedIn && !todayAttendance?.checkOutTime && (
                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                          <p className="text-yellow-800 text-sm font-medium">⏰ Check-in: {todayAttendance.checkInTime}</p>
                          <p className="text-yellow-700 text-xs mt-0.5">{todayAttendance.site} | {todayAttendance.labourType}</p>
                        </div>
                      )}

                      {!todayAttendance?.isCheckedIn && (
                        <div className="space-y-3">
                          <h3 className="font-semibold text-gray-800 text-sm">Kaam ki jaankari</h3>
                          {renderDropdown({ label: 'Site', name: 'site', value: attendanceData.site, options: dropdownData.siteNames, onChange: handleAttendanceDataChange, required: true })}
                          {attendanceData.site && userLocation && (
                            <div className={`text-xs p-2 rounded-lg ${isWithinRange ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                              {isWithinRange ? `✅ Range mein ho (${distanceToSite}m)` : `❌ ${distanceToSite}m door — 300m ke andar aao`}
                            </div>
                          )}
                          {renderDropdown({ label: 'Labour Type', name: 'labourType', value: attendanceData.labourType, options: dropdownData.laborTypes, onChange: handleAttendanceDataChange, required: true })}
                          {renderDropdown({ label: 'Category', name: 'category', value: attendanceData.category, options: dropdownData.categories, onChange: handleAttendanceDataChange })}
                          {attendanceData.labourType === 'Contractor Labour' && renderDropdown({ label: 'Contractor', name: 'contractorName', value: attendanceData.contractorName, options: dropdownData.contractorNames, onChange: handleAttendanceDataChange })}
                        </div>
                      )}

                      <button
                        className={`w-full py-3 px-4 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                          loading || !isWithinRange ? 'bg-gray-200 text-gray-400 cursor-not-allowed'
                          : todayAttendance?.isCheckedIn ? 'bg-blue-600 text-white hover:bg-blue-700'
                          : 'bg-green-600 text-white hover:bg-green-700'
                        }`}
                        onClick={handleSubmitAttendance}
                        disabled={loading || !isWithinRange}
                      >
                        {loading ? <><div className="w-4 h-4 border-2 border-gray-400 border-t-gray-600 rounded-full animate-spin"></div>Ho raha hai...</>
                         : !isWithinRange ? '🚫 Range se bahar ho'
                         : todayAttendance?.isCheckedIn ? '👋 Check Out Karo'
                         : '✅ Check In Karo'}
                      </button>
                    </>
                  )}
                  ...
                  {attendanceStep === 'done' && attendanceResult && (
                    <div className={`rounded-xl p-4 border ${
                      attendanceResult.status === 'checked-in' ? 'bg-green-50 border-green-200'
                      : attendanceResult.status === 'checked-out' ? 'bg-blue-50 border-blue-200'
                      : 'bg-yellow-50 border-yellow-200'
                    }`}>
                      <h3 className={`text-base font-bold mb-3 ${
                        attendanceResult.status === 'checked-in' ? 'text-green-800'
                        : attendanceResult.status === 'checked-out' ? 'text-blue-800'
                        : 'text-yellow-800'
                      }`}>
                        {attendanceResult.status === 'checked-in' && '✅ Check-in Ho Gaya!'}
                        {attendanceResult.status === 'checked-out' && '👋 Check-out Ho Gaya!'}
                        {attendanceResult.status === 'completed' && '📋 Aaj Ka Pura Hua'}
                      </h3>
                      {[
                        ['👤 Naam', attendanceResult.labour?.name],
                        ['🏗️ Site', attendanceResult.attendance?.site],
                        ['👷 Type', attendanceResult.attendance?.labourType],
                        ['🕐 Check-in', attendanceResult.attendance?.checkInTime],
                        attendanceResult.attendance?.checkOutTime && ['🕐 Check-out', attendanceResult.attendance?.checkOutTime],
                        attendanceResult.attendance?.workingHours && ['⏱️ Kaam', attendanceResult.attendance?.workingHours],
                      ...
                      <button className="mt-3 w-full py-2 text-sm bg-gray-100 text-gray-700 rounded-xl font-medium" onClick={() => { resetAll(); }}>
                        🔄 Naya Scan Karo
                      </button>
                    </div>
                  )}
                ...
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-bold text-gray-800">Naya Labour Register Karo</h2>
                    <p className="text-gray-500 text-xs mt-0.5">Details bharo + 2-3 photos lo</p>
                  </div>

                  <div>
                    <label className="block text-gray-700 font-medium text-sm mb-1">Naam <span className="text-red-500">*</span></label>
                    <input type="text" name="name" placeholder="Poora naam likhiye" value={formData.name} onChange={handleInputChange} disabled={loading}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:border-gray-500 outline-none transition-all disabled:opacity-50" />
                  </div>

                  <div>
                    <label className="block text-gray-700 font-medium text-sm mb-1">Phone <span className="text-red-500">*</span></label>
                    <input type="tel" name="phone" placeholder="10 digit number" value={formData.phone} onChange={handleInputChange} maxLength={10} disabled={loading}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-xl text-sm focus:border-gray-500 outline-none transition-all disabled:opacity-50" />
                  </div>

                  {renderDropdown({ label: 'Labour Type', name: 'labourType', value: formData.labourType, options: dropdownData.laborTypes, onChange: handleInputChange, required: true })}
                  {renderDropdown({ label: 'Category', name: 'category', value: formData.category, options: dropdownData.categories, onChange: handleInputChange })}
                  {renderDropdown({ label: 'Default Site', name: 'site', value: formData.site, options: dropdownData.siteNames, onChange: handleInputChange })}
                  {formData.labourType === 'Contractor Labour' && renderDropdown({ label: 'Contractor', name: 'contractorName', value: formData.contractorName, options: dropdownData.contractorNames, onChange: handleInputChange })}

                  {/* Photos preview */}
                  {capturedPhotos.length > 0 && (
                    <div className="bg-gray-50 rounded-xl p-3 border border-dashed border-gray-300">
                      <p className="text-gray-500 text-xs font-medium mb-2">Photos ({capturedPhotos.length}/3)</p>
                      <div className="grid grid-cols-3 gap-2">
                        {capturedPhotos.map((photo, index) => (
                          <div key={index} className="relative">
                            <img src={photo} alt={`Photo ${index+1}`} className="w-full h-16 object-cover rounded-lg border border-gray-200" />
                            <button className="absolute -top-1.5 -right-1.5 bg-red-500 text-white w-5 h-5 rounded-full flex items-center justify-center text-xs hover:bg-red-600 shadow"
                              onClick={() => removePhoto(index)} disabled={loading}>✕</button>
                            <span className="absolute bottom-1 left-1 bg-black bg-opacity-60 text-white text-xs px-1 rounded">{index+1}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <button
                    className={`w-full py-3 px-4 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                      loading || capturedPhotos.length < 2 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-gray-800 text-white hover:bg-gray-700 active:scale-[0.98]'
                    }`}
                    onClick={handleRegister}
                    disabled={loading || capturedPhotos.length < 2}
                  >
                    {loading ? <><div className="w-4 h-4 border-2 border-gray-400 border-t-gray-600 rounded-full animate-spin"></div>Register ho raha hai...</> : '✅ Register Karo'}
                  </button>

                  <div className="bg-amber-50 border-l-4 border-amber-400 rounded-r-lg p-3">
                    <p className="text-amber-900 font-semibold text-xs mb-1">📸 Photo tips</p>
                    {['Seedhi, baayi, dayi — teen angles se lo', 'Achhi roshni zaruri hai', 'Chehra saaf dikhna chahiye'].map(tip => (
                      <div key={tip} className="flex items-center gap-1.5 text-amber-800 text-xs mb-0.5">
                        <span className="text-green-600">✓</span>{tip}
                      </div>
                    ))}
                  </div>
              ...
              {/* Message */}
              {message && (
                <div className={`mt-3 p-3 rounded-xl flex items-start gap-2 text-sm border ${
                  messageType === 'success' ? 'bg-green-50 border-green-200 text-green-800'
                  : messageType === 'error' ? 'bg-red-50 border-red-200 text-red-800'
                  : 'bg-blue-50 border-blue-200 text-blue-800'
                }`}>
                  <span>{messageType === 'success' ? '✅' : messageType === 'error' ? '❌' : '⏳'}</span>
        ...
        <div className="text-center mt-5 text-gray-400 text-xs">
          🔒 Face Recognition + GPS • ⚡ Low Bandwidth Mode
        </div>
      </div>
    </div>
  );
