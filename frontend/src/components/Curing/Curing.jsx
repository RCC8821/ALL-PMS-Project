...
} from '../../features/Curing/fullkittingSlice'; // अपना सही path डालना
...
const Curing = () => {
  ...
  // Progress API - UID encode नहीं कर रहे (स्पेस वाला UID वैसा ही भेज रहे)
  const {
    data: progressData,
    ...
    selectedDaysCuringUID || undefined,
    {
  ...
  // Memoized completed days + debug logs
  const completedDays = useMemo(() => {
    const days = progressData?.completedDays || [];
    
    console.log('=== Progress Debug ===');
    console.log('Selected UID:', selectedDaysCuringUID);
    console.log('Progress data:', progressData);
    console.log('Completed days:', days);
    console.log('Loading:', progressLoading, progressFetching);
    console.log('Error:', progressError, progressErrorDetail?.data);
    console.log('===================');

    return days;
  }, [progressData, selectedDaysCuringUID, progressLoading, progressFetching, progressError]);
  ...
  const handleDaysAction = (uid) => {
    console.log("Opening Days modal for UID:", uid);
  ...
  return (
    <div className="p-5 bg-slate-50 min-h-screen">
      ...
      {/* Fullkitting Modal - Gallery + Camera दोनों बटन */}
      {showForm && (
        <div
      ...
      {/* Days Modal - Gallery + Camera दोनों बटन */}
      {showDaysModal && (
        <div
          className="fixed inset-0 bg-black/65 backdrop-blur-sm z-[1000] flex items-center justify-center p-4"
            ...
            <div className="p-6">
              {progressLoading || progressFetching ? (
                ...
                <div className="text-center text-red-600 py-8">
                  <p className="font-medium">Progress load करने में समस्या</p>
                  <p className="text-sm mt-2">
                    {progressErrorDetail?.data?.error || 'Unknown error'}
                  </p>
                </div>
              ) : (
                <>
                  ...
                  {!selectedDay ? (
                    ...
                    <div className="space-y-6">
                      ...
                      {/* Photos - Gallery + Camera दोनों बटन */}
                      <div className="space-y-6">
                        {[
